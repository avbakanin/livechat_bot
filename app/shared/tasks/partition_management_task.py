...
class PartitionManagementTask:
    ...
    async def _partition_loop(self):
        ...
        while self._running:
            try:
                ...
                logging.info(f"Partition management task: waiting {sleep_seconds:.0f} seconds until next partition creation")
                ...
                logging.info(f"Partition management task: waiting {sleep_seconds:.0f} seconds until next partition cleanup")
    ...
    def _get_next_25th(self) -> datetime:
        ...
        now = datetime.now()
        current_month_25th = now.replace(day=25, hour=5, minute=0, second=0, microsecond=0)

        if now.day >= 25:
            # Если уже прошло 25 число, берем следующий месяц
            if now.month == 12:
                next_month_25th = now.replace(year=now.year + 1, month=1, day=25, hour=5, minute=0, second=0, microsecond=0)
            else:
                next_month_25th = now.replace(month=now.month + 1, day=25, hour=5, minute=0, second=0, microsecond=0)
    ...
    def _get_next_1st(self) -> datetime:
        ...
        now = datetime.now()
        if now.month == 12:
            next_month_1st = now.replace(year=now.year + 1, month=1, day=1, hour=5, minute=0, second=0, microsecond=0)
        else:
            next_month_1st = now.replace(month=now.month + 1, day=1, hour=5, minute=0, second=0, microsecond=0)
    ...
    async def _create_next_month_partition(self):
        ...
        async with self.pool.acquire() as conn:
            try:
                ...
                logging.info(f"Partition management task: creating partition for {partition_date}")

                result = await conn.fetchval("SELECT public.ensure_messages_partition($1)", partition_date)
    ...
    async def _drop_old_partition(self):
        ...
        async with self.pool.acquire() as conn:
            try:
                ...
                logging.info(f"Partition management task: dropping partition for {partition_date}")

                result = await conn.fetchval("SELECT public.drop_messages_partition($1)", partition_date)
    ...
    async def force_create_partition(self, target_date: date):
        ...
        async with self.pool.acquire() as conn:
            try:
                logging.info(f"Force partition creation: creating partition for {target_date}")

                result = await conn.fetchval("SELECT public.ensure_messages_partition($1)", target_date)
    ...
    async def force_drop_partition(self, target_date: date):
        ...
        async with self.pool.acquire() as conn:
            try:
                logging.info(f"Force partition drop: dropping partition for {target_date}")

                result = await conn.fetchval("SELECT public.drop_messages_partition($1)", target_date)
    ...
    async def get_partition_status(self) -> list:
        ...
        async with self.pool.acquire() as conn:
            try:
                ...
                partitions = []
                for row in rows:
                    partitions.append({"schema": row["schemaname"], "table": row["tablename"], "size": row["size"]})

                logging.info(f"Partition management task: found {len(partitions)} partitions")
                return partitions

            except Exception as e:
                logging.error(f"Partition management task: error getting partition status: {e}")
